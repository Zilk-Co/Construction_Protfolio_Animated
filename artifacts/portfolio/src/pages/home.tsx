...
function TestimonialsSection() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/testimonials`)
      .then(r => r.json())
      .then(data => { setTestimonials(Array.isArray(data) ? data.filter((t: any) => t.published) : []); })
      .catch(() => {})
  ...
  return (
    <section className="py-24 px-6 bg-[hsl(220,18%,10%)]/70 border-b border-[hsl(220,15%,18%)] backdrop-blur-sm">
      <div className="max-w-screen-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.slice(0, 6).map((item: any, i: number) => (
            <motion.div
              key={item.id}
            ...
            >
              <div className="flex items-center gap-0.5 mb-4">
                {Array.from({ length: Math.min(item.rating || 5, 5) }).map((_: any, j: number) => (
                  <svg key={j} className="w-3.5 h-3.5 text-[hsl(38,72%,52%)]" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <p className="text-sm text-[hsl(220,12%,60%)] leading-relaxed mb-6 italic">"{item.content}"</p>
              <div>
                <p className="font-serif font-bold text-sm">{item.name}</p>
                <p className="text-[10px] text-[hsl(220,12%,45%)] mt-0.5">
